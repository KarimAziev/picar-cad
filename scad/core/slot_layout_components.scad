...
example_slots_2 = [smd_battery_holder_add_placeholder_size(["type", "custom",
                                                            ...
                                                            "bolt_visible_h",  2]),
                   smd_battery_holder_add_placeholder_size(["type", "custom",
                                                            "placeholder", "smd_battery_holder",
                                                            "rotation", 0,
                                                            "height", smd_battery_holder_height,
                                                            "length", smd_battery_holder_length,
...
module example_panels(nested_specs=[example_slots_2, example_slots],
                      align_to_axle = 1,
                      use_children=false,
                      direction = "ltr",
                      cols_direction = "ttb",
  ...
  total_sizes = [for (specs = nested_specs) get_total_size(specs,
                                                           direction=cols_direction)];

  widths = [for (size = total_sizes) size[0]];
  ...
  translate([0, 0, 0]) {
    for (i = [0 : len(nested_specs) - 1]) {
      let (specs = nested_specs[i],
           curr_size = total_sizes[i],
           curr_offset = curr_size[0],
           x_offset = sum(widths, i)) {
        translate([x_offset * (direction == "ltr" ? 1 : -1)
                   + curr_offset * (direction == "ltr" ? 1 : -1),
                   0,
                   0]) {
          example_panel(specs=specs,
                        debug=debug,
                        use_children=use_children,
