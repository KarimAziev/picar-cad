...
module servo_screws_hat(size, x_offset, d=steering_servo_screw_dia, thickness=servo_hat_thickness) {
  w= size[0];
  h = size[1];
...
module servo(size=servo_size,
  ...
  union() {
    ...
    translate([size[1] - gearbox_rad + 1.5, 0, -size[2] * 0.5]) {
      union() {
        color(servo_color) {
          hull() {
            cylinder(h = gearbox_height, r = gearbox_rad, center=true);
            translate([-gearbox_rad - 1, 0, 0]) {
              cylinder(h = gearbox_height, r = gearbox_rad * 0.4, center=true, $fn=50);
            }
          }
        }
        ...
        translate([0, 0, -gearbox_height * 0.5 - gear_height * 0.5]) {
          color("gold") {
            union() {
              cylinder(h = gear_height, r = gear_dia * 0.3, center = true, $fn=50);
              lower_h = (gear_height * 0.9);
              translate([0, 0, lower_h / 2]) {
                cylinder(h = lower_h, r = gear_dia * 0.5, center = true, $fn=50);
              }
            }
          }
        }
    ...
    color(servo_color) {
      translate([0, 0, -size[2] * 0.5 + screws_hat_z_offset]) {
        ...
        servo_screws_hat(size=[servo_hat_w, servo_hat_h], x_offset=x_offset);
      }
    }
  }
}

module servo_slot_2d(size=[steering_servo_slot_width, steering_servo_slot_height],
                     screws_dia=steering_servo_screw_dia,
