...
function calc_gear_teeth(pinion_d, tooth_pitch) = round((PI * pinion_d) / tooth_pitch);
function calc_actual_pitch(pinion_d, gear_teeth) = (PI * pinion_d) / gear_teeth;
function calc_rack_teeth(rack_len, actual_pitch) = floor(rack_len / actual_pitch);
...
function pinion_sync_params(pinion_d, tooth_pitch, rack_len) =
  pinion_add_rack_margin(pinion_compute_shared_params(pinion_d, tooth_pitch), rack_len);
