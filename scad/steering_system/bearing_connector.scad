  ...
  union() {
    color(connector_color) {
      linear_extrude(height = h) {
        ring_2d(r=steering_rack_link_bearing_d / 2,
                w=(steering_rack_link_bearing_outer_d - steering_rack_link_bearing_d) / 2,
                fn=360,
                outer=true);
      }
    }
...
module bearing_lower_connector(lower_d=steering_rack_link_bearing_outer_d,
                               lower_h=steering_rack_link_bearing_bearing_pin_base_h,
                               shaft_h=steering_rack_link_bearing_pin_height,
                               shaft_d=steering_rack_link_bearing_shaft_d,
                               stopper_h=steering_rack_link_bearing_stopper_height) {
...
module bearing_print_plate(step_offset=5) {
  translate([steering_rack_link_bearing_outer_d / 2 + step_offset, 0, 0]) {
...
module bearing_connector_assembly_view(animation_z_offset=5) {
  ...
  end_h = knuckle_rack_link_arm_height + animation_z_offset;
  base_h = steering_rack_link_bearing_bearing_pin_base_h;
