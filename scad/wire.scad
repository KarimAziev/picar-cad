...
module capsule(p1, p2, d=2, $fn_sph=32) {
  if (vlen(p2 - p1) > 1e-6)
    hull() {
      translate(p1) {
...
module wire_path(points, d=2, put_joints=true, $fn_sph=32, print_wire_len=false) {
  ...
  if (put_joints)
    for (i = [0 : len(points) - 1]) {
      translate(points[i]) sphere(d=d, $fn=$fn_sph);
    }
}

wire_path(points=[[50, 0, -150], [50, 50,-50], [0, 50, -50], [0, 0, -50], [0, 0, 0]],
          d=1.5,
          put_joints=true);